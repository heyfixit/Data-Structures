...
10. What is the runtime complexity of `DoublyLinkedList.delete`? O(1)
    ...
    Worst case on splice would be a case where you're splicing to the front and thus have to shift every further element
    over, so O(n).  A linked list deletes in constant time, O(1)
